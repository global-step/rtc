    ...
    return (
      <div className="App">
        ...
        {this.state.classes.map( (item, index) => {
          return <Class key={index} title={item.title} start={item.start} spaces={item.spaces} taken={item.taken} isDisabled={item.spaces === item.taken ? true : false} onClick={() => this.takeSpace(index)} />
        })}
      </div>
    );
  }
...
function Class(props){
  return(
    <div className="class">
      ...
      <a className="join" target="_blank" href="https://zoom.us/j/895806108">Join Class</a>
    </div>
  )
}
